...
const hideReview = async (id) => {
  try {
    await axios.get(base_url.value+'update/reviews/'+id, authHeader)
    geteReviews() // refresh the list
  } catch (err) {
    console.error(err)
  }
}
    ...
    <div class="reviews container mt-4">
      <div v-for="review in reviews" :key="review.id" class="card mb-3 shadow-sm">
        <div class="card-body">
          ...
          <div class="mb-0">
            <p>{{ review.comment }}</p> <button class="btn btn-danger float-end" @click="hideReview(review.id)">Hide</button>
          </div>
        </div>
      </div>
    </div>
